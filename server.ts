...
app.get('/publish', async (req, res) => {
    try {
        ...
        publisher.publish('products', JSON.stringify(product), (err: any, reply: any) => {
            if (err) {
...
app.post('/addData', async (req, res) => {
    try {
        ...
        publisher.set(key, value, (error: any, reply: any) => {
...
app.get('/getData', (req, res) => {
    try {
        var keyName = 'details'
        publisher.get(keyName, (error: any, value: any) => {
            if (error) {
                res.send(error)
            } else {
                res.send(value)
...
app.get('/list', (req, res) => {
    try {
        var data: any = []
        var data11: any = []
        let retrievedKeys: any = 0;
        publisher.keys('*', (error: any, keys: any) => {
            if (error) {
                res.send(error);
            } else {
                if (keys.length === 0) {
                ...
                keys.forEach((key: any) => {
                    publisher.get(key, (err: any, value: any) => {
                        if (err) {
                            res.send(err);
                        }
...
app.post('/delete', (req, res) => {
    try {
        ...
        publisher.del(keyName, (err: any, value: any) => {
            if (err) {
...
app.post('/pagination_list', (req, res) => {
    try {
        ...
        publisher.lrange('name1', startIndex, endIndex, (err: any, data: any) => {
            if (err) {
                res.json(err);
            } else {
                res.json(data);
...
app.post('/getList', (req, res) => {
    try {
        //get list
        publisher.lrange(req.body.listName, 0, -1, (error: any, response: any) => {
            if (error) {
                console.error(error);
                return;
            }
...
app.post('/deleteList', (req, res) => {
    try {
        ...
        //delete list
        publisher.del(req.body.listName, (error: any, response: any) => {
            if (error) {
                console.error(error);
                return;
            }
...
app.post('/getHash', (req, res) => {
    try {
        //get hash
        publisher.hgetall(req.body.hashName, (error: any, response: any) => {
            if (error) {
                console.error(error);
                return;
            }
...
app.post('/deleteHash', (req, res) => {
    try {
        //delete hash
        publisher.del(req.body.hashName, (error: any, response: any) => {
            if (error) {
                console.error(error);
                return;
            }
...
app.post('/searchHash_data', (req, res) => {
    try {
        ...
        publisher.hscan(hashKey, 0, 'MATCH', `*${searchField}*${searchValue}*`, (error: any, result: any) => {
            if (error) {
                console.error(error);
                return;
            }
...
app.post('/allHash', (req, res) => {
    try {
        ...
        publisher.keys('*', (error: any, keys: any) => {
            ...
            if(keys.length){
                keys.forEach((key: any) => {
                    publisher.hgetall(key, (error: any, value: any) => {
                        if (error) {
                            console.error(error);
                            return;
                        }
